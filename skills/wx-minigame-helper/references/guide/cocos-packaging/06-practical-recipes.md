...
4. 勾选 **分离引擎**（微信引擎插件）→ 插件清单覆盖的 chunk 重定向到微信共享插件（按微信规则仍计入首包限额；物理等模块不在清单内，**需靠功能裁剪减掉**，与分离引擎是组合技，见 `07-engine-plugin.md`）
5. 勾选 **引擎原生代码分包** → WASM 入子包
6. 勾选 **MD5 Cache** → 版本控制
7. 勾选 **引擎原生代码(wasm/asmjs)分包** → 所有开启了 WASM 的模块放入子包，减少主包占用。
...
| 勾了分离引擎，`cocos-js/` 里仍有大块引擎文件（如 `physics-*.js`） | 物理系列、skeletal-animation 等模块**不在官方插件清单内**，启用即 100% 进本地包；另有 ~440K 不在插件清单内的固定留守（animation 聚合与引擎主 index chunk） | 分离引擎前提下裁剪依然减包：裁掉不用且插件不覆盖的模块（物理全家优先）即可；`gfx-webgl2` 等后端开关对包体零影响（运行时动态选后端）；见 `07-engine-plugin.md` |
| 裁掉 spine/meshopt 后包体没变小，且 `cocos-js/assets/` 下对应 `.wasm` 还在 | spine 等 JS chunk 已命中插件（本地本就是 0 字节，裁掉收益为 0）；wasm 二进制**不跟随模块裁剪移除** | **能否删要看 import-map 壳映射**（不能 grep 本地产物，引用链在插件内部）：壳映射（`"../cocos-js/<前缀>-*.js": "plugin:cocos/..."`）仍在 → 必须保留（删了运行时报 wasm not found + Warning 1220）；壳映射已消失 → 可删（注意模块父开关关闭时版本子开关如 `spine-3.8` 可能未联动）；见 `07-engine-plugin.md` |
