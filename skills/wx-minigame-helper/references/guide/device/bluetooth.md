...
source: https://developers.weixin.qq.com/minigame/dev/guide/device/bluetooth.html
---
...
在小程序中，要使用蓝牙能力（Beacon 除外）必须首先调用 [wx.openBluetoothAdapter](<https://developers.weixin.qq.com/minigame/dev/api/device/bluetooth/wx.openBluetoothAdapter.html>) 初始化蓝牙适配器模块，其生效周期为调用 [wx.openBluetoothAdapter](<https://developers.weixin.qq.com/minigame/dev/api/device/bluetooth/wx.openBluetoothAdapter.html>) 至调用 [wx.closeBluetoothAdapter](<https://developers.weixin.qq.com/minigame/dev/api/device/bluetooth/wx.closeBluetoothAdapter.html>) 或小程序被销毁为止。只有在小程序蓝牙适配器模块生效期间，开发者才能够正常调用蓝牙相关的小程序 API，并收到蓝牙模块相关的事件回调（绑定监听不受此限制）。
