...
source: https://developers.weixin.qq.com/minigame/dev/guide/device/ble-mesh.html
---
...
在使用插件前，首先需要在小程序管理后台的「设置 - 第三方服务 - 插件管理」中添加插件，开发者可以登录小程序管理后台，通过 appid - wx013447465d3aa024 查找插件并添加。然后在 `app.json` 文件中声明指定版本的插件。关于插件的具体介绍可参考「[小游戏插件](<../base-ability/plugins.md>)」。

目前只有申请了「[工具—设备管理](<https://developers.weixin.qq.com/miniprogram/dev/framework/device/device-access>)」服务类目的小程序才能使用「小程序 BLE Mesh 插件」。
