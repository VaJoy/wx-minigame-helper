...
source: https://developers.weixin.qq.com/minigame/dev/guide/base-ability/HTTPDNS.html
---

# 移动解析HttpDNS

从基础库 [2.19.2](<../runtime/client-lib/compatibility.md>) 开始支持 

开发者调用[wx.request](<https://developers.weixin.qq.com/minigame/dev/api/network/request/wx.request.html>)时，可以开启移动解析HttpDNS服务。 该服务基于Http协议向服务商的DNS服务器发送域名解析请求，替代了基于DNS协议向运营商Local DNS发起解析请求的传统方式，可以避免Local DNS造成的域名劫持和跨网访问问题，解决移动互联网服务中域名解析异常带来的困扰。
...
## 小程序开发者使用移动解析说明
  ...
  2. 小程序调用[wx.request](<https://developers.weixin.qq.com/minigame/dev/api/network/request/wx.request.html>)，将enableHttpDNS参数设置为true，并在httpDNSServiceId参数中填入选用的服务商Service ID。
...
### 计费说明
  ...
  3. 若开发者的服务可用额度为0，仍在wx.request接口中声明使用服务商提供的移动解析能力时，会使用 localDNS 解析来兜底，并在 success 回调参数 exception.reasons ( reasons 是数组) 中返回 httpdns 欠费的错误信息和错误码，类似 `[{ "errMsg": "getDNSInfo:fail no enough httpdns quota", "errno": 602103 }]`。
