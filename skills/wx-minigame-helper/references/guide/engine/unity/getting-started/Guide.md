---
title: "Unity 游戏接入微信小游戏指南"
type: guide
category: guide/engine/unity/getting-started
source: https://developers.weixin.qq.com/minigame/dev/guide/game-engine/unity-webgl-transform/Design/Guide.html
---
...
![Unity快适配调优指南](https://res8.wxqcloud.qq.com.cn/wxdoc/e81581dd-9a70-4589-87cd-91f8e5471ad3.png)
...
> 相关手册：[兼容性评估](<Evaluation.md>) 、[推荐引擎版本](<https://developers.weixin.qq.com/minigame/dev/guide/game-engine/unity-webgl-transform/Design/UnityVersion.html>)、[更多小游戏成功转换案例](<ShowCase.md>) 、[技术常见问题QA](<DevelopmentQAList.md>)
...
> 相关手册：[资源按需加载概述](<../assets/ResourcesLoading.md>) 、[AA(Addressable) 进行资源按需加载](<../assets/UsingAddressable.md>) 、 [AB(AssetBundle)进行资源按需加载](<../assets/UsingAssetBundle.md>) 、[Instant Game 实践指南](<InstantGameGuide.md>)
...
> 相关手册：[后端服务指引](<../startup/BackendServiceStartup.md>) 、[网络通信适配](<../platform/UsingNetworking.md>)
...
> 相关手册：[资源部署与缓存](<../assets/DataCDN.md>) 、[资源缓存](<../assets/FileCache.md>) 、[iOS高性能模式](<../performance/iOSOptimization.md>)
...
> 相关手册：[开发错误调试与排查](<../profiling/DebugAndException.md>)
...
> 相关手册：[WX SDK 平台能力适配](<WX_SDK.md>) 、[屏幕适配](<../rendering/fixScreen.md>) 、 [输入法适配](<../platform/InputAdaptation.md>) 、 [排行榜与微信关系数据](<../platform/OpenData.md>)

​ Unity 游戏接入微信小游戏平台将获得微信提供的 API 以及开放能力，开发者根据需要进行按需接入。微信 API 支持的能力包括登录、设备（存储、震动）、开放数据、广告等等。

### 启动留存数据上报统计

> 相关手册：[启动留存数据上报统计](<../startup/ReportStartupStat.md>)

​ 在小游戏中玩家对启动时长与体验十分敏感（尤其从“广告”等买量场景进入的玩家），因此需要在必要的位置进行相关的数据上报，[Unity Loader 插件](<../assets/UsingLoader.md>)为游戏提供了一些基础上报数据，但游戏内部关键帧位置仍需要开发者自行上报，可参阅相关手册完成上报配置。
...
> 相关手册：[提升Unity WebGL游戏启动速度](<../startup/StartupOptimization.md>) 、[启动流程与时时序](<../startup/Startup.md>) 、 [首场景启动优化](<../startup/FirstSceneOptimization.md>)
...
> 相关手册：[使用预下载功能](<../assets/UsingPreload.md>)

​ 在 UnityLoader 加载过程中是存在CPU处理密集而 **网络空闲** 的情况，因此可以充分利用该时机进行一些必要的资源下载可有助于提升游戏场景等资源的载入速度。

### 运行性能及调优

> 相关手册：[优化Unity WebGL的运行性能](<../performance/OptimizationPerformence.md>)、[使用 Android CPU Profiler 性能调优](<../profiling/AndroidProfile.md>)、[使用 Unity Profiler 性能调优](<../profiling/UnityProfiler.md>)、[优化 Unity WebGL 内存](<../performance/OptimizationMemory.md>)

​ 游戏转化完成后仍需要对各项的性能指标进行量化测试。

### 压缩纹理资源

> 相关手册： [压缩纹理优化](<../assets/CompressedTexture.md>)
...
> 相关手册：[使用代码分包工具](<../performance/WasmSplit.md>)
...
> 相关手册：[启动封面](<../startup/CustomLoading.md>)
...
​ 审核发布可参考 MP 文档 [小游戏接入指南](<Guide.md>) 自助完成，游戏相关运行数据请前往 [微信公众平台](<https://mp.weixin.qq.com/>)。
