---
title: "定制微信小游戏的 URP 管线"
type: guide
category: guide/engine/unity/rendering
source: https://developers.weixin.qq.com/minigame/dev/guide/game-engine/unity-webgl-transform/Design/CustomURP.html
---
...
> ### # 本地化 URP 包
  ...
  * 导入本地的 `Universal RP`包 再次点击 左上角 `+` 号按钮，选择 `D:/custom_packages/com.unity.render-pipelines.universal@12.1.10/package.json`文件打开
  * 注意 需要先导入本地的 `Core RP Library`，然后再导入`Universal RP`. 顺序不能反! (否则会自动引入官方的`Core RP Library`包)
  * 当导入完成后，Unity Game 和 Scene 视图得渲染应该会恢复正常的
  * 此时在 `Package Manager` 窗口 选择 `Packages：In Project`, 在 `Packages - Other` 一栏会看到`Core RP Library` 和 `Universal RP` ![本地化](https://res8.wxqcloud.qq.com.cn/wxdoc/058a2118-0326-4501-8de0-92f0cfb5ad92.png)
...
> > #### # 微信 URP 定制版本的性能数据对比
  ...
  *     1. 使用 URP SimpleLit Shader 的测试场景，静置如下图： ![Case1](https://res8.wxqcloud.qq.com.cn/wxdoc/a850648d-a93a-4c32-81d8-f8f7fdde1ee3.jpeg)
...
定制URP | 59.9 | 38.2 | 8.87 | 45.5 | 23.1  
  ...
  *     2. 使用 URP Lit Shader 的测试场景，静置如下图： ![Case2](https://res8.wxqcloud.qq.com.cn/wxdoc/0bcfb992-2aaa-40fe-896b-ceb33f6c3c44.jpeg)
