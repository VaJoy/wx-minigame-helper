...
api: "pay_v2.cancelPay"
source: https://developers.weixin.qq.com/minigame/dev/api-backend/midas-payment/game_currency/api_pay_v2.cancelpay.html
---
...
amount | number | 是 | 本次退回的退游戏币的数量（支持部分、多次退）（新增字段） | -
bill_no | string | 是 | 扣除游戏币订单号，业务需要保证全局唯一，相同的订单号多次请求不会重复扣除；长度不超过 63，只能是数字、英文大小写字母及*-的组合；不能以下划线（*）开头（2.0 新增约束） | -
pay_bill_no | string | 是 | 扣除游戏币订单号（原 1.0 的 bill_no 字段）；与 [扣除游戏币](https://developers.weixin.qq.com/minigame/dev/api-backend/midas-payment/game_currency/api_pay_v2.pay) 接口中的 bill_no 对应 | -
