...
api: "storage.removeUserStorage"
source: https://developers.weixin.qq.com/minigame/dev/api-backend/data/api_storage.removeuserstorage.html
---
...
接口英文名：storage.removeUserStorage

本接口用于删除已经上报到微信的 key-value 数据。
...
```bash
POST https://api.weixin.qq.com/wxa/remove_user_storage?access_token=ACCESS_TOKEN&openid=OPENID&signature=SIGNATURE&sig_method=SIG_METHOD
```

### 云调用

- 调用方法：storage.removeUserStorage
- 调用说明：openid（用户唯一标识符）需作为参数传入，详情查看下文调用示例
...
// ...
// 方法返回 Promise
cloud.openapi.storage.removeUserStorage({
  openid: "xxx",
  key: ['a', 'b'],
});
```
