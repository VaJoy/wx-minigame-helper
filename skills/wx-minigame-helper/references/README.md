...
基于微信小游戏官方文档（[微信开放文档](https://developers.weixin.qq.com/minigame/dev/)）重新组织的本地知识库，共收录 **1224 篇**文档（924 篇前端 API 参考 + 256 篇开发指南 + 44 篇服务端 API 参考），面向 Agent 检索与人工阅读做了结构化优化。
...
├── api/             ← 前端 API 参考（924 篇，按功能域分组，文件名即 API 名；含云开发 wx.cloud，改编自官方小程序云托管文档，小游戏用法一致）
...
按功能域分为 24 个一级分类，每篇文档对应一个 API（类、方法、事件或属性），**文件名即 API 名**：
...
| `cloud/` | 云开发 `wx.cloud`（云托管调用：init / callContainer / connectContainer / Cloud 资源复用） |
