...
微信小游戏开发 SKILL：内置 1222 篇微信小游戏官方文档的本地知识库与配套 TypeScript 类型声明，让 AI 在回答小游戏开发问题、编写小游戏代码时**先查权威文档再作答**，避免凭记忆编造 API 行为。
...
- **微信云托管**：`wx.cloud.callContainer` / `connectContainer` 免域名配置调用云托管后端（HTTP + WebSocket，`references/api/cloud/`，5 篇）
...
├── references/          本地知识库（1222 篇，链接已全部本地化）
