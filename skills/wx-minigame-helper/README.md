...
微信小游戏开发 SKILL：内置 1224 篇微信小游戏官方文档的本地知识库与配套 TypeScript 类型声明，让 AI 在回答小游戏开发问题、编写小游戏代码时**先查权威文档再作答**，避免凭记忆编造 API 行为。

## 它能做什么

- **API 查询**：`wx.*` 任意接口的用法、参数、返回值、错误码、最低基础库版本
- **开放能力接入**：登录与用户体系、分享裂变、好友排行榜（开放数据域，含 Cocos 侧 SubContextView 应用）、广告变现、虚拟支付、订阅消息、视频号、游戏圈等
- **工程问题**：包体与启动优化、分包策略、性能排查（卡顿/内存/发热/弱网）、Worker 多线程、网络与本地存储
- **微信云托管**：`wx.cloud.callContainer` / `connectContainer` / `getTempFileURL` / `downloadFile` 调用云托管后端（HTTP + WebSocket + 对象存储，`references/api/cloud/`，7 篇）
- **Cocos 分包与远程包**：Cocos Creator 视角的 Asset Bundle 配置、微信小游戏分包、远程包（CDN）、运行时下载缓存（`references/guide/cocos-packaging/`，6 篇）
...
├── references/          本地知识库（1224 篇，链接已全部本地化）
...
└── scripts/types/       自动生成的 TypeScript 类型声明
    ├── *.d.ts           前端 wx.* 类型（381 个函数/属性、87 个类、457 个类成员）
